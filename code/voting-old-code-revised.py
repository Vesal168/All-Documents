...
# def calculate_weighted_probabilities(classifiers, weights, X):
#     total_prob = np.zeros((X.shape[0], len(CLASSES_LIST)))
...
# accuracies = np.array([accuracy for name, accuracy in top_two_classifiers])
# weights = accuracies / accuracies.sum()

# # Get weighted average probabilities
# weighted_probabilities = calculate_weighted_probabilities(top_classifiers, weights, test_features_pca)

# # Make final predictions based on weighted probabilities
# weighted_pred = np.argmax(weighted_probabilities, axis=1)

# # Evaluate the weighted average ensemble
# print("Weighted Average Ensemble Accuracy:", accuracy_score(y_test, weighted_pred))
# print(classification_report(y_test, weighted_pred, target_names=CLASSES_LIST))

# # Confusion matrix
# conf_matrix_weighted = confusion_matrix(y_test, weighted_pred)
# conf_matrix_normalized_weighted = conf_matrix_weighted.astype(float) / conf_matrix_weighted.sum(axis=1)[:, np.newaxis] * 100

# # Plot confusion matrix for weighted average ensemble
# plt.figure(figsize=(10, 8))
# sns.heatmap(conf_matrix_normalized_weighted, annot=True, fmt='.2f', cmap='Blues', xticklabels=CLASSES_LIST, yticklabels=CLASSES_LIST)
# plt.xlabel('Predicted Labels')
# plt.ylabel('True Labels')
...
for name, clf in classifiers.items():
    ...
    # Confusion matrix of each ML classifiers
    conf_matrix = confusion_matrix(y_test, pred)
    conf_matrix_normalized = conf_matrix.astype(float) / conf_matrix.sum(axis=1)[:, np.newaxis] * 100

    # Plot confusion matrix of each ML classifiers
    plt.figure(figsize=(10, 8))
    sns.heatmap(conf_matrix_normalized, annot=True, fmt='.2f', cmap='Blues', xticklabels=CLASSES_LIST, yticklabels=CLASSES_LIST)
    plt.xlabel('Predicted Labels')
    plt.ylabel('True Labels')
...
# Sort ML classifiers by accuracy and select the top 2
sorted_classifiers = sorted(classifier_accuracies.items(), key=lambda item: item[1], reverse=True)
top_two_classifiers = sorted_classifiers[:2]

# Print the top two ML classifiers
print("Top two classifiers:")
for name, accuracy in top_two_classifiers:
    print(f"{name}: {accuracy:.4f}")

# Create a dictionary of the top two ML classifiers
top_classifiers = [(name, classifiers[name]) for name, _ in top_two_classifiers]
