...
# # Save the ResNet model weights
# resnet_model.save_weights('resnet_model_soft_voting2.weights.h5') # Save model
# print("ResNet model weights saved successfully.")

# # Extract features
# train_features = resnet_model.predict(x_train_reshaped)
# test_features = resnet_model.predict(x_test_reshaped)
...
# # Apply PCA for dimensionality reduction with the appropriate number of components
...
# # Calculate weights based on classifier accuracies
# total_accuracy = sum(classifier_accuracies.values())
# weights = {name: accuracy / total_accuracy for name, accuracy in classifier_accuracies.items()}

# # Apply Soft Voting Ensemble
# weighted_avg_pred = np.zeros((test_features_pca.shape[0], len(CLASSES_LIST)))
# for name, clf in classifiers.items():
#     weighted_avg_pred += weights[name] * clf.predict_proba(test_features_pca)

# # Select the class with the highest probability as the final prediction
# final_pred = np.argmax(weighted_avg_pred, axis=1)

# # Evaluate the soft voting ensemble
# print("Soft Voting Ensemble Accuracy:", accuracy_score(y_test, final_pred))
# print(classification_report(y_test, final_pred, target_names=CLASSES_LIST))

# # Confusion matrix for soft voting ensemble
# conf_matrix_soft_voting = confusion_matrix(y_test, final_pred)
# conf_matrix_normalized_soft_voting = conf_matrix_soft_voting.astype(float) / conf_matrix_soft_voting.sum(axis=1)[:, np.newaxis] * 100

# # Plot confusion matrix for soft voting ensemble
# plt.figure(figsize=(10, 8))
# sns.heatmap(conf_matrix_normalized_soft_voting, annot=True, fmt='.2f', cmap='Blues', xticklabels=CLASSES_LIST, yticklabels=CLASSES_LIST)
# plt.xlabel('Predicted Labels')
# plt.ylabel('True Labels')
# plt.title('Confusion Matrix - Soft Voting Ensemble')
# plt.show()

# # Save the soft voting ensemble model
# joblib.dump(final_pred, 'soft_voting_ensemble_model.pkl')
# print("Soft Voting Ensemble model saved successfully.")
...
# Sort classifiers by accuracy and select the top 2
sorted_classifiers = sorted(classifier_accuracies.items(), key=lambda item: item[1], reverse=True)
top_two_classifiers = sorted_classifiers[:2]

# Print the top two classifiers
print("Top two classifiers:")
for name, accuracy in top_two_classifiers:
    print(f"{name}: {accuracy:.4f}")

# Create a dictionary of the top two classifiers
top_classifiers = [(name, classifiers[name]) for name, _ in top_two_classifiers]

# Create a voting classifier with the top two classifiers
voting_clf = VotingClassifier(estimators=top_classifiers, voting='soft')
...
print("Top Two Classifier Voting Accuracy:", accuracy_score(y_test, voting_pred))
...
plt.xlabel('Predicted Labels')
plt.ylabel('True Labels')
plt.title('Confusion Matrix - Top Two Classifier Voting')
plt.show()
