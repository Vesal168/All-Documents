...
# from keras.layers import Conv2D, MaxPooling2D, BatchNormalization, Dropout, Reshape, GRU, Dense, Input
...
# x = GRU(200, return_sequences=False, activation='relu')(x)
# x = Dropout(0.3)(x)
...
# output_layer = Dense(len(CLASSES_LIST), activation='softmax')(x)

# cnn_gru_model = Model(inputs=input_layer, outputs=output_layer)

# cnn_gru_model.compile(loss='sparse_categorical_crossentropy', optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), metrics=['accuracy'])
...
# history = cnn_gru_model.fit(x_train_reshaped, y_train, epochs=300, batch_size=4, shuffle=True, validation_data=(x_test_reshaped, y_test))
...
# # Evaluate the model
# test_loss, test_accuracy = cnn_gru_model.evaluate(x_test_reshaped, y_test)
...
# feature_extractor = Model(inputs=cnn_gru_model.input, outputs=cnn_gru_model.layers[-4].output)
...
# svm_grid.fit(train_features_pca, y_train)
# svm_pred = svm_grid.predict(test_features_pca)
# print("SVM Accuracy:", accuracy_score(y_test, svm_pred))
# print(classification_report(y_test, svm_pred, target_names=CLASSES_LIST))
