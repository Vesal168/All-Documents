...
# Create a BaggingClassifier for each top classifier
bagging_classifiers = {}
for name, clf in top_classifiers:
    bagging_clf = BaggingClassifier(base_estimator=clf, n_estimators=50, random_state=42)
    bagging_clf.fit(train_features_pca, y_train)
    bagging_classifiers[name] = bagging_clf
    joblib.dump(bagging_clf, f'bagging_{name}_model.pkl')
    print(f"Bagging {name} model saved successfully.")

    # Evaluate the BaggingClassifier
    bagging_pred = bagging_clf.predict(test_features_pca)
    accuracy = accuracy_score(y_test, bagging_pred)
    print(f"Bagging {name} Accuracy:", accuracy)
    print(classification_report(y_test, bagging_pred, target_names=CLASSES_LIST))

    # Confusion matrix
    conf_matrix_bagging = confusion_matrix(y_test, bagging_pred)
    conf_matrix_normalized_bagging = conf_matrix_bagging.astype(float) / conf_matrix_bagging.sum(axis=1)[:, np.newaxis] * 100

    # Plot confusion matrix for BaggingClassifier
    plt.figure(figsize=(10, 8))
    sns.heatmap(conf_matrix_normalized_bagging, annot=True, fmt='.2f', cmap='Blues', xticklabels=CLASSES_LIST, yticklabels=CLASSES_LIST)
    plt.xlabel('Predicted Labels')
    plt.ylabel('True Labels')
    plt.title(f'Confusion Matrix - Bagging {name}')
    plt.show()
