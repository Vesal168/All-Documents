...
from keras.layers import (
    ...
    Dropout, Dense, Input, Concatenate, Activation, GlobalAveragePooling2D
)
...
def densenet_block(inputs, num_filters, kernel_size, growth_rate):
    concatenated_inputs = inputs
    for _ in range(num_filters):
        x = BatchNormalization()(concatenated_inputs)
        x = Activation('relu')(x)
        x = Conv2D(growth_rate, kernel_size=kernel_size, padding='same')(x)
        x = Dropout(0.2)(x)
...
x = densenet_block(x, num_filters=4, kernel_size=(3, 3), growth_rate=12)
x = MaxPooling2D(pool_size=(2, 2))(x)
x = Dropout(0.2)(x)

x = densenet_block(x, num_filters=4, kernel_size=(3, 3), growth_rate=24)
x = MaxPooling2D(pool_size=(2, 2))(x)
x = Dropout(0.2)(x)

x = densenet_block(x, num_filters=4, kernel_size=(3, 3), growth_rate=36)
