...
stacking_clf = StackingClassifier(
    estimators=[('svm', svm_clf), ('knn', knn_clf), ('rf', rf_clf)], final_estimator=LogisticRegression()
)

# Train the stacking classifier
stacking_clf.fit(train_features_pca, y_train)

# Evaluate the stacking classifier
stacking_pred = stacking_clf.predict(test_features_pca)
print("Stacking Classifier Accuracy:", accuracy_score(y_test, stacking_pred))
print(classification_report(y_test, stacking_pred, target_names=CLASSES_LIST))

# Confusion matrix
conf_matrix_stacking = confusion_matrix(y_test, stacking_pred)
conf_matrix_normalized_stacking = conf_matrix_stacking.astype(float) / conf_matrix_stacking.sum(axis=1)[:, np.newaxis] * 100

# Plot confusion matrix for stacking classifier
plt.figure(figsize=(10, 8))
sns.heatmap(conf_matrix_normalized_stacking, annot=True, fmt='.2f', cmap='Blues', xticklabels=CLASSES_LIST, yticklabels=CLASSES_LIST)
